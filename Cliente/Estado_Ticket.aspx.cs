...
public partial class Cliente_Estado_Ticket : System.Web.UI.Page
{
    ...
    private void listarestadostickets()
    {
        this.__mensaje.Value = "";

        this.__pagina.Value = "";

        for (int i = 1; i < this.Tabletickets.Rows.Count; i++)
        {
            this.Tabletickets.Rows[i].Cells.Clear();
        }
        ...
        try
        {
            ...
            {
                System.Data.DataTable dt = servidor.consultar("[dbo].[listarestadostickets]").Tables[0];
                ...
                else
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        for (int j = 0; j < 3; j++)//Cabecera de la tabla
                        {
                            ...
                            switch (j)
                            {
                                ...
                                case 2:
                                    ...
                                    b.Text = "ESTADO TICKET";

                                    b.ForeColor = System.Drawing.Color.Green;

                                    b.ToolTip = "Seleccionar Estado Ticket";
                                    ...
                                    b.OnClientClick = "Datos_Ticket(" + dt.Rows[i]["ID"].ToString() + ",'" + dt.Rows[i]["ESTADO TICKET"].ToString() + "');";
                        ...
                        this.Tabletickets.Rows.Add(tRow);
                    }
    ...
    private void mantenimientoestadoticket(int _ID_ESTTICKET, string _DESCRIPCION, string operacion)
    {
        try
        {
            servidor.cadenaconexion = Ruta;
            ...
            {
                servidor.ejecutar("[dbo].[mantenimientoestadoticket]",
                                    false,
                                    _ID_ESTTICKET,
                                    _DESCRIPCION.Trim(),
                                    operacion,
                ...
                if (servidor.getRespuesta() == 1)
                {
                    ...
                    ShowMessage(servidor.getMensaje(), "Estado_Ticket.aspx");
    ...
    {
        if (Convert.ToInt32(this.ID_ESTTICKET.Value.Trim()) == 0)
        {
    ...
    protected void Page_init(object sender, EventArgs e)
    {
        ...
        listarestadostickets();

    }
    protected void btnRegistrar_Click(object sender, EventArgs e)
    {
        Boolean ok;
        ok = rfvTICKET.IsValid;
        if (ok == false)
        {
            return;
        }

        mantenimientoestadoticket(Convert.ToInt32(ID_ESTTICKET.Value.Trim()),
            Convert.ToString(TICKET.Text.Trim()),
            "N");

        this.Page.RegisterStartupScript("", "<script> Limpiar_Datos_Ticket(); </script>}");
    }
