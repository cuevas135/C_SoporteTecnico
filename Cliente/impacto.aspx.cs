...
public partial class Cliente_impacto : System.Web.UI.Page
{
    ...
    private void listarimpactos()
    {
        this.__mensaje.Value = "";

        this.__pagina.Value = "";

        for (int i = 1; i < this.Tableimpactos.Rows.Count; i++)
        {
            this.Tableimpactos.Rows[i].Cells.Clear();
        }
        ...
        try
        {
            ...
            {
                System.Data.DataTable dt = servidor.consultar("[dbo].[listarimpactos]").Tables[0];
                ...
                else
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        for (int j = 0; j < 3; j++)//Cabecera de la tabla
                        {
                            ...
                            switch (j)
                            {
                                ...
                                case 2:
                                    ...
                                    b.OnClientClick = "Datos_Impacto(" + dt.Rows[i]["ID"].ToString() + ",'" + dt.Rows[i]["IMPACTO"].ToString() + "');";
                        ...
                        this.Tableimpactos.Rows.Add(tRow);
                    }
    ...
    private void mantenimientoimpacto(int _ID_IMP, string _DESCRIPCION, string operacion)
    {
        try
        {
            servidor.cadenaconexion = Ruta;
            ...
            {
                servidor.ejecutar("[dbo].[mantenimientoimpacto]",
                                    false,
                                    _ID_IMP,
                                    _DESCRIPCION.Trim(),
                                    operacion,
                ...
                if (servidor.getRespuesta() == 1)
                {
                    ...
                    ShowMessage(servidor.getMensaje(), "impacto.aspx");
    ...
    {
        if (Convert.ToInt32(this.ID_IMP.Value.Trim()) == 0)
        {
    ...
    {
        Boolean ok;
        ok = rfvIMPACTO.IsValid;
        if (ok == false)
        {
            return;
        }

        mantenimientoimpacto(Convert.ToInt32(ID_IMP.Value.Trim()),
            Convert.ToString(IMPACTO.Text.Trim()),
            "N");

        this.Page.RegisterStartupScript("", "<script> Limpiar_Datos_Impacto(); </script>}");
    }
